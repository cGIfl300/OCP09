    ...
    if request.method == "POST":
        ...
        if user_to_follow is not None:
            ...
            if user_exist(user_to_follow):
                if follow(user_to_follow, request):
...
def un_follow(request):
    ...
    if request.method == "POST":
        ...
        if user_to_unfollow is not None:
            ...
            object_user_to_unfollow = User.objects.filter(
                username__iexact=user_to_unfollow
            )[0]
            UserFollows.objects.filter(
                user=actual_user, followed_user=object_user_to_unfollow
            ).delete()
            return render(
                request, "unfollow.html", {"user_to_unfollow": user_to_unfollow}
            )
